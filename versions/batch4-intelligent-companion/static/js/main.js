        ...
        taskEl.innerHTML = `
            ...
            ${!task.completed ? `
                <div class="task-actions">
                    <button class="task-action-btn snooze" data-task-id="${task.id}" title="Snooze to tomorrow">💤</button>
                    <button class="task-action-btn reschedule" data-task-id="${task.id}" title="Smart reschedule">📅</button>
                    <button class="task-action-btn breakdown" data-task-id="${task.id}" title="Break down task">🔨</button>
                </div>
            ` : ''}
        `;

        taskEl.querySelector('.task-checkbox').addEventListener('click', () => {
