...
try:
    ...
    # Silent fallback for local development (JSON storage works fine)
...
# Print storage mode (only in production or if explicitly enabled)
if DATABASE_ENABLED:
    print("💾 Using PostgreSQL database")
# JSON storage is the default for local development - no message needed
...
def create_task():
    ...
    task = {
        ...
        'description': task_data.get('description', ''),  # Batch 5
        'completed': False,
        ...
        'category': task_data.get('category', ''),  # Batch 5: work, study, personal, etc.
        'status': task_data.get('status', 'not_started'),  # not_started, in_progress, completed
        'tags': task_data.get('tags', []),  # Batch 5
...
def complete_habit(habit_id):
    """Mark habit as complete for a specific date (or today if not specified)"""
    data = load_data()
    ...
    for habit in data['habits']:
        if habit['id'] == habit_id:
            if not habit['completions'].get(target_date, False):
...
def uncomplete_habit(habit_id):
    """Unmark habit as complete for a specific date (or today if not specified)"""
    data = load_data()
    request_data = request.get_json() or {}
    target_date = request_data.get('date', datetime.now().date().isoformat())
    ...
    for habit in data['habits']:
        if habit['id'] == habit_id:
            if habit['completions'].get(target_date, False):
                del habit['completions'][target_date]
                habit['streak'] = calculate_streak(habit['completions'])
                save_data(data)
                return jsonify(habit), 200
            return jsonify(habit), 200
...
def update_habit(habit_id):
    ...
    for habit in data['habits']:
        if habit['id'] == habit_id:
            ...
            if 'icon' in habit_data:
                habit['icon'] = habit_data['icon']
            if 'frequency' in habit_data:
                habit['frequency'] = habit_data['frequency']
            if 'frequency_count' in habit_data:
                habit['frequency_count'] = habit_data['frequency_count']
            if 'category' in habit_data:
                habit['category'] = habit_data['category']
            if 'streak' in habit_data:
                habit['streak'] = habit_data['streak']
            if 'archived' in habit_data:
                habit['archived'] = habit_data['archived']
...
def calculate_perfect_days(data):
    ...
    for date_str in dates:
        date_tasks = [t for t in tasks if t.get('due_date') == date_str or 
                     (t.get('completed_at') and t.get('completed_at')[:10] == date_str)]
        if not date_tasks:
            continue
...
# ============================================
# BATCH 6: POWER FEATURES API
# ============================================

@app.route('/api/search', methods=['GET'])
def global_search():
    """Global search across tasks, habits, tags, dates"""
    ...
    for task in data.get('tasks', []):
        ...
        if any(query in tag.lower() for tag in task.get('tags', [])):
            match = True
            reasons.append('tag')
        if query in task.get('due_date', ''):
            match = True
            reasons.append('date')
        
        if match:
            task_copy = task.copy()
            task_copy['match_reasons'] = reasons
            results['tasks'].append(task_copy)
    
    # Search habits
    for habit in data.get('habits', []):
        if query in habit.get('name', '').lower() or query in habit.get('category', '').lower():
            results['habits'].append(habit)
    ...
    for task in data.get('tasks', []):
        for tag in task.get('tags', []):
            if query in tag.lower():
                all_tags.add(tag)
    results['tags'] = list(all_tags)
    
    # Search dates
    date_matches = set()
    for task in data.get('tasks', []):
        if task.get('due_date') and query in task.get('due_date'):
            date_matches.add(task.get('due_date'))
    results['dates'] = list(date_matches)
...
def get_templates():
    """Get all templates"""
    # Templates are stored client-side in localStorage
    # This endpoint could be used for syncing templates
    return jsonify({'templates': []}), 200

@app.route('/api/projects', methods=['GET'])
def get_projects():
    """Get all projects/folders"""
    # Projects stored client-side
    return jsonify({'projects': []}), 200

@app.route('/api/activity', methods=['GET'])
def get_activity():
    """Get activity log"""
    # Activity log stored client-side
    return jsonify({'activities': []}), 200

# ============================================
# BATCH 7: AUTHENTICATION & ENTERPRISE FEATURES
# ============================================
...
def auth_google():
    ...
    return jsonify({'message': 'Google OAuth not yet implemented'}), 501

@app.route('/api/auth/apple', methods=['GET'])
def auth_apple():
    """Initiate Apple Sign In"""
    # TODO: Implement Apple Sign In
    return jsonify({'message': 'Apple Sign In not yet implemented'}), 501

@app.route('/api/auth/github', methods=['GET'])
def auth_github():
    """Initiate GitHub OAuth"""
    # TODO: Implement GitHub OAuth
    return jsonify({'message': 'GitHub OAuth not yet implemented'}), 501
...
def send_magic_link():
    ...
    return jsonify({'message': 'Magic link sent (mock)'}), 200
...
def send_otp():
    ...
    return jsonify({'session_id': session_id, 'message': 'OTP sent (mock)'}), 200
...
def refresh_token():
    ...
    token = request.headers.get('Authorization', '').replace('Bearer ', '')
    new_token = f"refreshed-{datetime.now().timestamp()}"
...
def calendar_connect():
    ...
    return jsonify({'message': 'Calendar OAuth not yet implemented'}), 501
...
def calendar_sync():
    ...
    return jsonify({'events_synced': 0, 'message': 'Sync not yet implemented'}), 200

@app.route('/api/calendar/events', methods=['POST'])
def create_calendar_event():
    """Create event in Google Calendar"""
    # TODO: Create event in Google Calendar
    return jsonify({'message': 'Event creation not yet implemented'}), 501

@app.route('/api/calendar/events/<int:event_id>', methods=['PUT'])
def update_calendar_event(event_id):
    """Update event in Google Calendar"""
    # TODO: Update event
    return jsonify({'message': 'Event update not yet implemented'}), 501

@app.route('/api/calendar/events/<int:event_id>', methods=['DELETE'])
def delete_calendar_event(event_id):
    """Delete event from Google Calendar"""
    # TODO: Delete event
    return jsonify({'message': 'Event deletion not yet implemented'}), 501

# Export/Import Endpoints
@app.route('/api/export/tasks', methods=['GET'])
def export_tasks():
    """Export tasks as CSV"""
    # Tasks are exported client-side
    return jsonify({'message': 'Use client-side export'}), 200

@app.route('/api/export/calendar', methods=['GET'])
def export_calendar():
    """Export calendar as ICS"""
    # Calendar is exported client-side
    return jsonify({'message': 'Use client-side export'}), 200

@app.route('/api/export/notes', methods=['GET'])
def export_notes():
    """Export notes as Markdown"""
    # Notes are exported client-side
    return jsonify({'message': 'Use client-side export'}), 200
...
if __name__ == '__main__':
    ...
    if is_production:
        ...
        print("=" * 50)
        print("🚀 Flow App - Production Mode")
        ...
        if port != 5000:
            print("⚠️  Port 5000 is busy, using port", port)
        ...
        print("=" * 50)
        print("🚀 Flow App - Local Development Mode")
        print(f"📱 Local: http://localhost:{port}")
        print(f"🌐 Network: http://{local_ip}:{port}")
        print("=" * 50)
        print("💡 Access from other devices on same WiFi:")
        print(f"   → Open http://{local_ip}:{port} in browser")
        print("=" * 50)
